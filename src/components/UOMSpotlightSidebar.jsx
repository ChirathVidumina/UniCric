...
import { Target, Star, Award, Zap, ChevronRight, Flame } from 'lucide-react';

export default function UOMSpotlightSidebar() {
  const batsmen = [
    { name: 'Sathira Vikasitha', runs: 48 },
    { name: 'Muftee Mysan', runs: 33 },
    { name: 'Behan Wickramasinghe', runs: 10 },
    { name: 'Devdun Nethusahan', runs: 9 }
  ];

  const bowlers = [
    { name: 'Kevindu Perera', wkts: 3, runs: 16 },
    { name: 'Behan Wickramasinghe', wkts: 2, runs: 7 },
    { name: 'Kelum Hirudika', wkts: 1, runs: 10 },
    { name: 'Sanithu Wijerathne', wkts: 1, runs: 15 }
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <h4 style={{ fontSize: '1.1rem', fontWeight: '900', color: 'white', marginBottom: '0.5rem' }}>Sathira Vikasitha</h4>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: '1.5', marginBottom: '1rem' }}>
            Played a crucial knock of <span style={{ background: 'rgba(255,255,255,0.1)', padding: '0.15rem 0.35rem', borderRadius: '4px', color: 'white', fontWeight: '700' }}>48 runs</span> off 63 balls, smashing 6 boundaries.
          </p>
          <div style={{
          ...
          {batsmen.map((bat, idx) => (
            <div 
              ...
              style={{ 
                ...
                padding: '0.75rem 0',
                borderBottom: idx !== batsmen.length - 1 ? '1px solid var(--border-color)' : 'none'
              }}
            >
      ...
      <div className="content-card" style={{ marginBottom: 0, padding: '1.25rem', borderTop: '3px solid var(--accent-blue)', boxShadow: '0 4px 15px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {bowlers.map((bowl, idx) => (
            <div 
              key={bowl.name} 
              style={{ 
                display: 'flex', justifyContent: 'space-between', alignItems: 'center', 
                padding: '0.75rem 0',
                borderBottom: idx !== bowlers.length - 1 ? '1px solid var(--border-color)' : 'none'
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span style={{ 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                  <span style={{ fontSize: '0.9rem', fontWeight: '900', color: 'var(--accent-blue)' }}>{bowl.wkts}</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                  <span style={{ fontSize: '0.85rem', fontWeight: '700', color: 'white' }}>{bowl.runs}</span>
                  <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: '800' }}>RUNS</span>
                </div>
              </div>
            </div>
